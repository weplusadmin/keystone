...
var s3 = new AWS.S3({apiVersion: '2006-03-01', accessKeyId: process.env.S3_KEY, secretAccessKey: process.env.SE_SECRET});
...
function s3image (list, path, options) {
	...
	if (!keystone.get('s3image config')) {
		throw new Error(
			'Invalid Configuration\n\n'
			+ 'CloudinaryImage fields (' + list.key + '.' + this.path + ') require the "s3image config" option to be set.\n\n'
 ...
 */
s3image.prototype.getFolder = function () {
	var folder = null;
	if (keystone.get('s3image folders') || this.options.folder) {
		if (typeof this.options.folder === 'string') {
			folder = this.options.folder;
		} else {
			var folderList = keystone.get('s3image prefix') ? [keystone.get('s3image prefix')] : [];
...
s3image.prototype.addToSchema = function (schema) {
	...
	var paths = this.paths = {
		...
		fileKey: this.path + '.fileKey',
		publicUrl: this.path + '.publicUrl',
		signedUrl: this.path + '.signedUrl'
	};
	...
	var src = function (item, options) {
		...
		if (!('fetch_format' in options) && keystone.get('s3image webp') !== false) {
			options.fetch_format = 'auto';
		}
		if (!('progressive' in options) && keystone.get('s3image progressive') !== false) {
			options.progressive = true;
		}
		if (!('secure' in options) && keystone.get('s3image secure')) {
			options.secure = true;
		}
 ...
 */
s3image.prototype.getData = function (item) {
	var value = item.get(this.path);
	return typeof value === 'object' ? value : {};
};

s3image.prototype._originalGetOptions = s3image.prototype.getOptions;

s3image.prototype.getOptions = function () {
...
function validateInput (value) {
	...
	if (typeof value === 'object' && value.publicUrl) return true;
 ...
 * Deprecated
 */
s3image.prototype.inputIsValid = function () {
	return true;
};
...
s3image.prototype.updateItem = function (item, data, files, callback) {
	...
	if (typeof value === 'object') {
		...
		if ( value != '' && value != undefined) {
			item.set(this.path, JSON.parse(value));
		} else {
			item.set(this.path, '');
		}
...
/**
	...
	getFilename below
*/
s3image.prototype.retryFilename = prototypeMethods.retryFilename;

/**
	Gets a filename for uploaded files based on the adapter options
*/
s3image.prototype.getFilename = prototypeMethods.getFilename;

s3image.prototype.fileExists = function (filename, callback) {
...
s3image.prototype.getRequestHandler = function (item, req, paths, callback) {
	...
	return function () {
		...
		if (req.body && req.body[paths.select]) {
			...
			var tp = keystone.get('s3image prefix') || '';
			var imageDelete;
			if (tp.length) {
				tp += '_';
			}
			...
			if (keystone.get('s3image folders')) {
